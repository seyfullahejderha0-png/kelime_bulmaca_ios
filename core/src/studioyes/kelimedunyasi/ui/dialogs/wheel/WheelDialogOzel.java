...
public class WheelDialogOzel extends BaseDialog {

    private TextButton spinButton;
    private LuckyWheelOzel luckyWheel;
    ...
    public WheelDialogOzel(float width, float height, BaseScreen screen, final int seviye) {
        ...
        setTitleLabel( LanguageManager.format("level", seviye)+"\n"+ LanguageManager.get("extra_words_complete"));
        ...
        luckyWheel = new LuckyWheelOzel(createGiftBoxContainer, screen.wordConnectGame.resourceManager);
        ...
        actualSpinCount = GameConfig.ALLOWED_SPIN_COUNTOzel;

        spinButton.addListener(new ChangeListener() {
            @Override
            public void changed(ChangeEvent event, Actor actor) {
                determineSpin();
                new Ayarlar().setTamamlananLevelOdulAldi(seviye);
            }
        });
    }
    ...
    private void changeSpinButton(){
        if (actualSpinCount > 0){
            ...
            if(videoIcon == null && screen.wordConnectGame.adManager != null && screen.wordConnectGame.adManager.isRewardedAdEnabledToSpinWheel()) {
                ...
                spinButton.setHeight(spinButton.getHeight()+100);
                spinButton.setY(spinButton.getY()-50);
            ...
            spinButton.setText("Ads\n" + LanguageManager.format("spin_again", actualSpinCount));
    ...
    private void determineSpin(){
        ...
        if(actualSpinCount == GameConfig.ALLOWED_SPIN_COUNTOzel){
            spin();
        }else{
    ...
    private void sayGoodbye(){
        ...
        //todo burası son ekran
        rewardLabel.setText(LanguageManager.format("spin_tomorrowOzel", GameConfig.ALLOWED_SPIN_COUNTOzelSeviye));
